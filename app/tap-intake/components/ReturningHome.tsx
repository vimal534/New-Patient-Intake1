  ...
  return (
    <PhoneFrame>
      <div className="flex flex-1 flex-col overflow-y-auto px-6 pb-6 pt-6">
        <div className="text-[11px] font-semibold uppercase tracking-wide text-muted-2">{nextVisit.date}&apos;s visit</div>
        ...
        <div className="mt-2 flex items-start gap-3">
          ...
          <div>
            <h1 className="text-[26px] font-bold leading-tight text-ink">How&apos;s {child.name} doing?</h1>
            <p className="mt-1 text-sm text-muted">
              {child.age} years · {nextVisit.visitType} with {nextVisit.provider}
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 flex items-center gap-4 rounded-2xl border border-line bg-white p-4 shadow-[0_2px_8px_rgba(0,0,0,0.05)]">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-sm text-muted">
              {nextVisit.location} · {nextVisit.provider}
            </div>
        ...
        {/* One merged, divided block instead of individually boxed rows —
            ...
            list in ui.tsx's SectionShell, for the same reason: several
            same-weight scannable facts read better as one list than as a
        ...
        {/* No flex-1 spacer here (the previous version had one) — it was
            pushing the CTA to the very bottom of the scroll area, leaving
            a large empty gap on most viewports and forcing a scroll to
            reach it even though the content above easily fits in view. */}
        <div className="mt-8">
          <PrimaryButton onClick={onStart}>
            Tell us what&apos;s going on <span aria-hidden="true">→</span>
          </PrimaryButton>
          <p className="mt-3 text-center text-xs text-muted">About 2 min · we save as you go</p>
        </div>
      </div>
    </PhoneFrame>
  );
